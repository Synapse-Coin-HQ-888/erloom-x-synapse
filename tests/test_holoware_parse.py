...
from errloom.synapse.synapse import (
    ClassSpan,
    ContextResetSpan,
    EgoSpan,
    Synapse,
...
from errloom.synapse.synapse_parser import (
    _build_class,
    _build_context,
    _build_ego_or_sampler,
    _build_obj,
    filter_comments,
    SynapseParser,
    parse_span_tag,
...
def _load_and_print_synapse(code: str) -> Synapse:
    """Parse a Synapse script, pretty-print it, and return the parsed structure."""
    syn = SynapseParser(code).parse()
    logger.info(Panel(code, title="Synapse Code", expand=False, border_style="cyan"))
    logger.info(syn.to_rich())
    return syn
...
class SynapseParserTest(ErrloomTest):
    def test_ware_parser_basic(self):
        ...
        for code, expected_details in test_cases:
            with self.subTest(code=code):
                spans = _load_and_print_synapse(code).spans
    ...
    def test_parser_implicit_system_ego(self):
        ware = _load_and_print_synapse("some text")
    ...
    def test_parser_context_reset_clears_ego(self):
        ...
        ware = _load_and_print_synapse(code)
    ...
    def test_parser_span_before_ego_raises_error(self):
        with self.assertRaises(ValueError):
            _load_and_print_synapse("<|MyClass|>")
        with self.assertRaises(ValueError):
            _load_and_print_synapse("<|my_obj|>")
        with self.assertRaises(ValueError):
            _load_and_print_synapse("<|fence=run|>")

    def test_parser_empty_tag(self):
        spans = _load_and_print_synapse("<||>").spans
...
class CompressorSynapseTest(ErrloomTest):
    def test_parser_compressor_synapse(self):
        ware = _load_and_print_synapse(COMPRESSOR_SYNAPSE)
        spans = ware.spans
